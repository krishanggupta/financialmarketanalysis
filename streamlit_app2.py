...
from returns_main import Intraday_data_files
...
with tab3:
    ...
    with tab4:
        ...
        # Add functionality for user to add custom function and save it.
        ...
        selected_sessions = st.multiselect("Select Session",mysessions)

        # Add custom session via button
        default_text=f'Distribution of Returns {y} with interval of {x}'
        finalname=default_text
        final_list=[]
        
        if st.button('Make Custom Session'):
            # User input
            new_option_original = st.text_input("Enter a new option to add:",value='0-5 ET,0-9 ET')
            ...
            new_session_name=st.text_input("Enter your desired session name:",value='MySession1')
            custom_session_name=f'{new_session_name} + {new_option_original}'
        else:
            custom_session_name=""

        # Get combination of selected inputs
        # Values
        filter_sessions1=[]
        filter_sessions1_names=[]
        for my_selection in selected_sessions:
            ...
            filter_sessions1_names.append(my_selection)

        filter_sessions2=[]
        filter_sessions2+=final_list
        filter_sessions2_names=[custom_session_name]

        filter_sessions=list(set(filter_sessions1+filter_sessions2))
        filter_sessions_names=list(set(filter_sessions1_names+filter_sessions2_names))
        if filter_sessions:
            st.text(f'Selected Sessions: {filter_sessions}')
        finalname=f'{default_text} for session:{filter_sessions1_names}, {filter_sessions2_names[0]}:{list(set(filter_sessions2))} ET'
        ...
        if filter_sessions:
            ...
            finalname+=f' for dates:{finalstart}-{finalend}'
        ...
        # Display the probability stats dataframe
